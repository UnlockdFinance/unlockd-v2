...
contract BasicWalletRegistry is IDelegationWalletRegistry, Ownable {
  ...
  modifier onlyFactoryOrOwner() {
    if (_msgSender() != _walletFactory && owner() != _msgSender())
  ...
  function setWallet(
    ...
    if (_wallet == address(0))
      revert Errors.DelegationWalletRegistry__setWallet_invalidWalletAddress();
    if (_owner == address(0))
      revert Errors.DelegationWalletRegistry__setWallet_invalidOwnerAddress();
